...
export function FormModal({
  ...
  // Default footer if onSubmit is provided and no custom footer
  const defaultFooter = onSubmit && !footer ? (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className={cn(
        "bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-xl shadow-2xl w-full flex flex-col", 
        maxWidth
      )}
      style={{ maxHeight: 'calc(100vh - 64px)' }}
      >
        {/* Header */}
        <div className="flex items-start justify-between px-6 py-5 border-b border-neutral-200 dark:border-neutral-800 flex-shrink-0">
          <div className="flex-1 pr-4">
            <h3 className="text-lg font-semibold text-neutral-900 dark:text-white leading-tight">{title}</h3>
            {description && <p className="text-sm text-neutral-600 dark:text-neutral-400 mt-2 leading-relaxed">{description}</p>}
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-lg text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800 hover:text-neutral-700 dark:hover:text-neutral-300 transition-colors flex-shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
